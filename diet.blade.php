...
<div class="main">
<u1>
                <li ><a href="http://127.0.0.1:8000/home2">Home</a></li>
                @if (Route::has('login'))
                @auth
                @else
...
<div style="width: 450px; height:300px; position: absolute;
    left: 625px;
    top: 120px;">
        <h2 style="color:white;">BMI</h2>
        <input type="text" id="txtFirstNo" placeholder="Height" onkeyup="sum()" />
        <input type="text" id="txtSecondNo" placeholder="Weight" onkeyup="sum()" />
        <br />
        <div style="padding-top:10px">
        <div style=" color:white; position:absolute; left:-71px;">Result:</div>
            <input type="text" id="txtResult"  style=" color:black; position: absolute;
    right: 224px;
}"/>
        </div>
		</div>
    ...
    <script type="text/javascript">
        function sum() {
            ...
            var result =parseInt(txtSecondNo)/( (parseInt(txtFirstNo)/100)* (parseInt(txtFirstNo)/100));
            if (!isNaN(result)) {
                document.getElementById('txtResult').value = result;
            }
            if (  result<=18.5)
            window.location.href = "{{ url('/WeightGain') }}"
            else if (  result>=18.5 && result<=24.9)
            window.location.href = "{{ url('/Fitness') }}"
            else if (  result>=25.0 && result<=29.9)
            window.location.href = "{{ url('/Weightloss') }}"
            else if (  result>=30)
            window.location.href = "{{ url('/Obesity') }}"
        }
    </script>
</body>
</html>
