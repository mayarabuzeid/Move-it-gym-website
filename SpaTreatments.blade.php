...
<style>
header {
            background-image: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), url(/spa.jpg);
            width: 100%;
            height: 900px;
...
<table id="customers">
  ...
  <?php //SpaController::Check($tr->Spa_Id);
  //$cap = DB::table('spa')->where('Spa_Id',$tr->Spa_Id)->value('Spa_capacity');
  if($tr->Spa_capacity<=0){
    unset($tr);
  }
  ?>
  @endforeach
...
@endif
 <tr>
<td>{{ $tr->Spa_Treatment }}</td>
<td>{{ $tr->Spa_Duration }}</td>
<td>{{ $tr->Spa_Price }}</td>
<td>{{ $tr->Spa_Date }}</td>
<td>{{ $tr->Spa_Time }}</td>
<td>{{ $tr->Spa_capacity }}</td>
<td><a href="{{action('App\Http\Controllers\SpaController@Booking',$tr->Spa_Id)}}"> <input type="submit" class="btn" value="Subscribe"></a></td>
</tr>
@endforeach
</table>
<script>
