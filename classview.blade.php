...
<table id="customers">
  <tr>
    ...
    <th>Class Time</th>
    <th>Class Trainer</th>
    <th>No of classes(1 to 12)</th>
    <th>Subscribe</th>
  </tr>
  @foreach ($classes as $class)
<tr>
  <?php if($class->Class_type ==0){
    $type="Offline";
  }
  else if($class->Class_type ==1)
  $type="Online";
  else{
  $type="Online/Offline";}
    ?>
...
<?php $trainer= ClassesController::trainer($class->Class_Id );?>
<td>Coach.{{ $trainer}}</td>
<form action="{{action('App\Http\Controllers\ClassesController@subscribe',$class->Class_Id)}}">
<td><input type="number" id="quantity" name="quantity" min="1" max="12" value="1"></td>
<td><input type="submit" class="btn" value="Subscribe"></td>
</form>
</tr>
@endforeach
</table>
